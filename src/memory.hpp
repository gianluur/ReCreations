...
// The standard library functions are only included if not in RECREATIONS_ONLY
// mode.
...
// @brief Provides fundamental memory management (allocation/deallocation) and
// manipulation utilities, often mirroring standard C library functions.
namespace memory {
...
// @param ptr Pointer to the memory block previously allocated by allocate or
// reallocate.
// @param size The new size in bytes for the memory block.
// @return A pointer to the newly allocated/reallocated memory block, or nullptr
...
// @brief Copies a block of memory from a source to a destination, safely
...
// @param src Pointer to the value used to fill the memory block (only the first
// byte is used).
...
// @return true if the blocks are identical (all size bytes match), false
// otherwise.
...
// @brief Performs an unconditional cast to an rvalue reference, enabling move
// semantics.
...
// Note: Since all functions are declared inline, their definitions must be
// provided here in the header file.
...
#ifndef RECREATIONS_ONLY
  // Copy bytes from src to dest. Behavior is undefined if memory blocks
  // overlap.
...
#ifndef RECREATIONS_ONLY
  // Copy bytes from src to dest, handling potential overlap between the memory
  // blocks.
...
inline void* memory::set(void* dest, const void* src, const uint64 size) {
  ...
  return memset(dest, *(static_cast<const unsigned char*>(src)), size);
#else
  // In recreation mode, return null.
  return nullptr;
#endif
...
#ifndef RECREATIONS_ONLY
  // Compare the specified number of bytes. Return true if they are identical
  // (result is zero).
...
inline T&& memory::pass_ownership(T& t) noexcept {
  // Cast an lvalue reference to an rvalue reference to enable moving the
  // object's resources.
